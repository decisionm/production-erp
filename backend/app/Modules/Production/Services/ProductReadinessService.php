...
class ProductReadinessService
{
     ...
     * }
     */
    public function assess(Item $item, ?Warehouse $warehouse = null, ?WorkCenter $workCenter = null): array
    {
        $severities = config('production.readiness.checks', []);
        $failures = $this->failures($item, $warehouse, $workCenter);
     ...
     */
    private function failures(Item $item, ?Warehouse $warehouse, ?WorkCenter $workCenter): array
    {
        $failures = [];
