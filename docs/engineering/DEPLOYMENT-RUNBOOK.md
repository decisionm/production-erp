...
| 12 | #190 Phase 7 Regression + hardening | #189 | one additive column widening (quality/scrap note) | none | No behaviour change on live: the two normalised figures (`accepted_quantity`, `actual_pieces`) are normalised TO the shape live already emits, so live output is byte-identical and only dev changes. Reports gain `row_cap`/`truncated`. **CI gains an `app-mysql` job — the repo owner must add it to branch protection for it to gate merges** |
| 13 | #191 Phase 7.6 Configuration Lifecycle | #190 | one additive migration: created_by/updated_by on ten configuration tables (per-column guard, reversible) | none | **No route is exposed and no entity is wired**, so the only user-facing change is the eleven active-flag refusals: a RETIRED master (mould, scrap reason, vendor, customer, GL account, leave type, salary component, routing, SPC characteristic, asset, measuring instrument) and an inactive item/warehouse can no longer be chosen for NEW work on the paths named in the PR; history still renders them, and the pickers no longer offer them. **Before deploying, check live for rows that are inactive but still in daily use** — the refusal is correct, but it is a behaviour change on live data. The audit trail starts recording configuration changes (activitylog was installed and idle until now) |
| 14 | Phase 7.5 Store → Production material flow (PR TBD) | #191 | three additive tables: material_requests, material_request_lines, store_issues (+lines, bag scans). No backfill, nothing dropped | none | **A store issue stops being a consumption.** Stock issued to production now stands in Production/WIP until a batch consumes it, so the raw-material store's balance falls at ISSUE and production's consumption draws from WIP. The Tally stock reconciliation understands this and will NOT report an open issue as drift. **Before deploying, confirm the live WIP warehouse row is the one intended** — the resolver identifies it by its own identity, and fails closed if it cannot. Day Bin rows and history are untouched; its three unused write doors are closed |
