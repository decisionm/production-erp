...
| 11 | Phase 6 Purchase chain + PO→Tally staged (PR TBD) | #188 | `2026_08_18_100000` purchase_order_revisions · `100001` PO close/cancel + tally_staging + vendors.tally_ledger_name · `100002` goods_receipt_note_lines.stock_movement_id (all additive, no backfill) | **`TALLY_SYNC_PURCHASE_ORDERS_ENABLED` — leave UNSET/false.** Setting it true is an OWNER-GATED act (Q35(d)) and the first live PO post must be attended. The purchase ledger is a Ledger Mappings role (Purchase), not an env key | PO lifecycle actions (amend/close/cancel) + show/trace on the procurement screens; vendors gain a Tally ledger name field (empty until someone fills it — an unmapped vendor is REFUSED, never guessed); with the flag off nothing is staged and each order says 'not sent — disabled'. Agent 0.3.9 is NOT published, so the live agent (0.3.8) is unaffected and would refuse a Purchase Order entry it cannot build — which cannot arise while the flag is off |
