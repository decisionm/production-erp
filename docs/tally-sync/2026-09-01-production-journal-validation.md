...
### 7. Three of the 34 are not production journals — PENDING Q92
...
The ERP side validates its own voucher PAYLOAD against a trimmed two-voucher
fixture (`backend/tests/Feature/TallySync/ProductionStockJournalMatchesRealTallyTest.php`).
This file is about the layer after it: the tally-sync-agent's XML BUILDER,
which is what actually produces the bytes Tally receives. A payload can be
correct and the XML still wrong — that gap is what the 07-Aug-2026 outage was.

Neither replaces the other, and they use different fixtures on purpose: the
backend's is trimmed for readability, this one is a whole voucher because the
boilerplate is part of what is being matched.
