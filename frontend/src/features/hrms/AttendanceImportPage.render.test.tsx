...
function run(overrides: Partial<AttendanceImport> = {}): AttendanceImport {
    return {
        ...
        issue_count: 594,
        open_count: 3,
        counts: { open: 3, in_no_out: 1, out_no_in: 0, no_punch: 1, unknown_employee: 1, hours_unclear: 0, worked_on_week_off: 0, report_changed: 0, resolved: 2, clean: 1233 },
...
describe('AttendanceImportPage — one answer for one kind of problem', () => {
    it('offers to mark every unpunched day absent, naming the count', () => {
        const html = render(
            ...
            { issue: 'no_punch' },
            run({ counts: { open: 366, in_no_out: 0, out_no_in: 0, no_punch: 366, unknown_employee: 0, hours_unclear: 0, worked_on_week_off: 0, report_changed: 0, resolved: 0, clean: 1240 } }),
    ...
    it('offers a shift end for every missing out-punch', () => {
        const html = render(
            ...
            { issue: 'in_no_out' },
            run({ counts: { open: 223, in_no_out: 223, out_no_in: 0, no_punch: 0, unknown_employee: 0, hours_unclear: 0, worked_on_week_off: 0, report_changed: 0, resolved: 0, clean: 1240 } }),
    ...
    it('never offers to answer for people the master does not have', () => {
        const html = render(
            ...
            run({ counts: { open: 12, in_no_out: 0, out_no_in: 0, no_punch: 0, unknown_employee: 12, hours_unclear: 0, worked_on_week_off: 0, report_changed: 0, resolved: 0, clean: 1240 } }),
            { lines: page([], 12) },
        );
    ...
    it('disables Apply with the open count, and shows the download only once applied', () => {
        ...
        const applied = render(
            ...
            run({
                status: 'applied',
                open_count: 0,
                counts: { open: 0, in_no_out: 0, out_no_in: 0, no_punch: 0, unknown_employee: 0, hours_unclear: 0, worked_on_week_off: 0, report_changed: 0, resolved: 5, clean: 1233 },
