 ...
 *
 * The first line that NAMES something gives the row its label and the rest
 * become a count, because a cell has to stay one line at any width and a
 * requisition of nine materials cannot spell all nine. First rather than "most
 * important": nothing in the data ranks lines, and inventing a rank would put a
 * product at the top of the cell on the app's judgement rather than the
 * requester's order. First-that-names rather than simply first, because an
 * opening line whose item the master no longer serves would otherwise render a
 * dash beside a count — strictly less than the line count it replaced.
 *
